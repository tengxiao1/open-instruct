...
for split_var in polaris_only_mix; do
    split_value="${!split_var}"
    exp_name=sft_olmo2_lc_wildchat_8192_${model}_${split_var}_${RANDOM}
    if [ "$model" == "qwen3" ]; then
        model_name_or_path=hamishivi/qwen3_openthoughts2
        chat_template_name=tulu_thinker
        add_bos=False
    elif [ "$model" == "olmo2_lc" ]; then
        model_name_or_path=/weka/oe-adapt-default/jacobm/checkpoints/olmo2-7B-sft/rl-sft/olmo2-7B-lc-OT3-456k-R1_wildchat_p1_2_oasst
