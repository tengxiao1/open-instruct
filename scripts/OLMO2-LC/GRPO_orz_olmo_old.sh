export orz_only_mix="TTTXXX01/rlvr_orz_math_57k_copy 1.0"
for model in olmo2_lc; do
for split_var in orz_only_mix; do
    split_value="${!split_var}"
    exp_name=GRPO_olmo_thinker_r1_style_jup_${model}_${split_var}_${RANDOM}
    if [ "$model" == "qwen3" ]; then
...
python mason.py \
        ...
        --wandb_project_name GSPO \
        --fill_completions False  
done
done
